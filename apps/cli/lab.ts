...
if(process.argv.includes('--live'))throw new Error('Use the approved manifest: node --import tsx apps/cli/experiment.ts preflight|execute MANIFEST [PRIVATE_OUTPUT]');
const counts=[3,5,8];const root=await mkdtemp(join(tmpdir(),'multi-llm-lab-'));
...
try{
  for(const count of counts){
    ...
    delete env.MODEL_PROVIDER;delete env.MODEL_NAME;delete env.MODEL_BASE_URL;delete env.LLM_API_KEY;env.ALLOW_LIVE_MODELS='0';
    const cluster=await startCluster(env,true),client=new CoreClient(cluster.base,cluster.token);
    try{
      const settings=SettingsSchema.parse({debounceMs:0,maxCoalesceMs:0,directedDebounceMs:0,arbitrationMs:10,postGapMs:30,agentCooldownMs:40,replyGraceMs:0,idleMs:3000,selfWakeMinMs:60000,selfWakeMaxMs:60000,maxMessages:count*2,maxCalls:3000,maxDurationMs:120000,reviewTtlMs:30000,memoryEvery:100});
      const created=await fetch(cluster.base+'/v1/sessions',{method:'POST',headers:{authorization:'Bearer '+cluster.token,'content-type':'application/json','idempotency-key':randomUUID()},body:JSON.stringify({title:`Mock ${count}-agent session`,participants:slots.map((s,i)=>({slot:s.id,characterId:['sora','nagi','rin'][i%3],profileId:'mock'})),settings})});
      ...
      for(;;){
        ...
        if(quietSince&&Date.now()-quietSince>4000){await post(`/v1/sessions/${id}/pause`,{});break;}
      ...
      const summary={agents:count,mode:'mock',messages:snapshot.session.botMessages,participatingAgents:authors.size,status:snapshot.session.activity,stopReason:snapshot.session.stopReason,calls:snapshot.session.calls,metrics:diagnostic.metrics};
      ...
      await writeFile(join(artifacts,`mock-${count}-metrics.json`),JSON.stringify(summary,null,2));
