...
try {
  const diagnostic=await diagnosticCommand(process.argv.slice(2));
  if(diagnostic)console.log(JSON.stringify({...diagnostic,warning:'Private diagnostic artifact; do not publish.'}));
  else {const result=await operatorCommand(process.argv.slice(2));console.log(result.body);if(!result.ok)process.exitCode=1;}
}catch(error){
