...
export async function diagnosticCommand(args:string[],env:NodeJS.ProcessEnv=process.env,fetcher:typeof fetch=fetch,options:{maxBytes?:number}={}):Promise<{kind:string;records:number}|null>{
  ...
  if(!Number.isSafeInteger(maxBytes)||maxBytes<1024||maxBytes>MAX_DIAGNOSTIC_BYTES)throw new Error('DIAGNOSTIC_BYTE_BUDGET_INVALID');
  ...
  try{
    ...
    if(response.status===413){await response.body?.cancel();throw new Error('DIAGNOSTIC_SIZE_LIMIT');}
    ...
    try{for(;;){const {done,value}=await reader.read();if(done)break;bytes+=value.length;
      if(bytes>maxBytes)throw new Error('DIAGNOSTIC_SIZE_LIMIT');writeFileSync(fd,value);}
      fsyncSync(fd);
