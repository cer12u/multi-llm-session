...
export async function startCluster(input:NodeJS.ProcessEnv=process.env,quiet=false):Promise<{base:string;token:string;stop:()=>Promise<void>;children:ChildProcess[]}> {
  ...
  const file=env.APP_CONFIG?JSON.parse(readFileSync(env.APP_CONFIG,'utf8')) as {workerSlots?:{id:string;tokenEnv:string}[]}:{};
