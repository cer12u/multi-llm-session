...
import {diagnosticTables} from '../../packages/storage-sqlite/diagnostic-migration.js';
...
export function asV8Fixture(db:Database.Database):void {
  if(db.inTransaction||db.pragma('user_version',{simple:true})!==9)throw new Error('EXPECTED_ISOLATED_V9_FIXTURE');
  db.transaction(()=>{
