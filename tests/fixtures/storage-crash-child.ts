...
function die():never{writeFileSync(path+'.crash-'+phase,phase,{flag:'wx',mode:0o600});process.kill(process.pid,'SIGKILL');throw new Error('Fixture kill did not occur');}
...
const app=buildServer(service,{timers:false});await app.listen({host:'127.0.0.1',port:0});
const address=app.server.address();if(!address||typeof address==='string')throw new Error('No crash fixture port');
config.publicOrigin=`http://127.0.0.1:${address.port}`;process.send?.({ready:true,base:config.publicOrigin});
