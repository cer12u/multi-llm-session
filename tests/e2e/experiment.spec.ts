...
test('R10-EXPERIMENT-E2E: preflight has no requests; an approved bounded run retains private evidence and rejects unsafe execution',async()=>{
  ...
  const provider=createServer(async(req,res)=>{
    ...
    res.setHeader('content-type','application/json');res.end(JSON.stringify({choices:[{finish_reason:'stop',message:{content:JSON.stringify(output)}}],usage:{prompt_tokens:19,completion_tokens:11}}));
  });
  ...
  const invoke=(args:string[])=>new Promise<{status:number;out:string;err:string}>((resolve,reject)=>{
    const child=spawn(process.execPath,['--import','tsx','apps/cli/experiment.ts',...args],{env,stdio:['ignore','pipe','pipe']});let out='',err='';
  ...
  try{
    ...
    writeFileSync(file,JSON.stringify(manifest));const ready=await invoke(['preflight',file]);expect(ready.status,ready.err+ready.out).toBe(0);expect(JSON.parse(ready.out).status).toBe('READY');expect(calls).toBe(0);
    const run=await invoke(['execute',file,output]);expect(run.status,run.err+run.out).toBe(0);
    ...
    mkdirSync('artifacts',{recursive:true});writeFileSync('artifacts/experiment-e2e.json',JSON.stringify({mode:'synthetic-http-e2e',owners:owners.size,calls,preflightNoSend:true,privateFiles:true,refusesOverwrite:true,semanticAcceptance:'NOT_EVALUATED'}));
