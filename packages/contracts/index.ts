...
export const ModelProfileSchema = z.object({
  id: Slug, provider: z.enum(['mock', 'ollama', 'openai']), model: z.string().min(1).max(200),
...
export const SettingsSchema = z.object({
  ...
  maxDurationMs: z.number().int().min(1000).max(21600000).default(900000),
...
}).strict().refine(v => v.selfWakeMaxMs >= v.selfWakeMinMs, 'selfWakeMaxMs must not be smaller');
...
export const ErrorCodeSchema = z.enum(['API_ERROR', 'RATE_LIMIT', 'TIMEOUT', 'FORMAT_ERROR', 'CONTEXT_LIMIT', 'CANCELLED', 'CONFIG_ERROR']);
...
export type PublicMessage = {
  id: string; sessionId: string; revision: number; authorId: string | null; authorName: string;
...
export type PublicAgent = { id: string; slot: string; characterId: string; characterVersion: number; name: string;
  presentationRef: string | null; profileId: string; enabled: boolean; status: string };
export type PublicSession = { id: string; title: string; lifecycle: Lifecycle; activity: Activity; revision: number;
  ...
  calls: number; botMessages: number; settings: Settings; mode: 'mock' | 'live' };
export type Snapshot = { session: PublicSession; agents: PublicAgent[]; messages: PublicMessage[]; cursor: string };
