import { AppError, ensure, type Context } from '../contracts/index.js';
...
export const visibleInputSQL = (alias='i') => `(${alias}.kind='message' OR EXISTS(SELECT 1 FROM source_items s
  ...
  AND EXISTS(SELECT 1 FROM (SELECT ? AS owner) access
    WHERE (s.audience_json='null' OR EXISTS(SELECT 1 FROM json_each(s.audience_json) WHERE value=access.owner))
    AND EXISTS(SELECT 1 FROM source_versions v WHERE v.source_id=s.id AND v.version=${alias}.version AND v.enabled=1
      AND (v.audience_json='null' OR EXISTS(SELECT 1 FROM json_each(v.audience_json) WHERE value=access.owner))))))`;
...
export function inputHigh(store:Store,sessionId:string,agentId?:string):number {
  ...
  const high=store.get<{n:number}>(`SELECT COALESCE(MAX(i.id),0) n FROM agent_input_log i WHERE i.session_id=?${suffix}`,
    ...agentId?[sessionId,agentId]:[sessionId])!.n;
  if(!agentId)return high;
  const cursor=store.get<{observed_input:number;memory_input:number}>('SELECT observed_input,memory_input FROM agent_input_cursors WHERE agent_id=?',agentId);
  return Math.max(high,cursor?.observed_input??0,cursor?.memory_input??0);
}
export function sourceChunk(row:SourceRow,agentId:string,offset=0):SourceChunk {
  let through=Math.min(row.text.length,offset+1600);
  ensure(offset===0||!/[\uDC00-\uDFFF]/.test(row.text.charAt(offset)),409,'SOURCE_RESYNC_REQUIRED');
  if(through<row.text.length&&/[\uD800-\uDBFF]/.test(row.text.charAt(through-1)))through--;
...
export function lookupSource(store:Store,sessionId:string,agentId:string,id:string,cursor:string|null):SourceChunk {
  ...
  if(cursor){
    ...
    try{value=JSON.parse(Buffer.from(cursor,'base64url').toString('utf8'));}catch{throw new AppError(422,'INVALID_SOURCE_CURSOR');}
