...
const versionTables:Record<number,string[]>={
  10:['budget_windows','call_budgets'],
...
function check(db:Database.Database):number {
  ...
  if(version>=10){
    const indexes=new Set((db.prepare("SELECT name FROM sqlite_master WHERE type='index'").all() as {name:string}[]).map(x=>x.name));
    if(['budget_current_window','budget_session_windows','budget_scope_calls','budget_window_calls'].some(name=>!indexes.has(name)))throw new Error('STORAGE_SCHEMA_INCOMPLETE');
  }
