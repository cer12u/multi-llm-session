...
import {ModelWireOutputSchemas} from '../../packages/models/model-protocol.ts';
...
const proxy=createServer(async(req,res)=>{
  ...
  const kind=context.delivery?.purpose==='memory'?'memory':system.includes('Write only your proposed utterance')?'draft':system.includes('Reconsider your private candidate')?'review':system.includes('Observe the supplied delivery')?'observe':'decide';
  const serialized=JSON.stringify(body);for(const forbidden of ['expectedVersion','observationId','sessionId','profileHash','privateState'])if(serialized.includes(forbidden))throw new Error('INTERNAL_BINDING_LEAK');
  const record={kind,owner:context.self.name,startedMs:Date.now()-started,requestHash:fingerprint(payload),requestBytes:bytes,formatSchema:typeof body.format==='object',request:body,response:null,summary:null};records.push(record);
  ...
  const upstream=httpRequest('http://127.0.0.1:11434'+req.url,{method:'POST',headers:{'content-type':'application/json','content-length':payload.length}},reply=>{
    ...
    reply.on('end',()=>{
      ...
      if(raw?.message?.content)try{const decoded=JSON.parse(raw.message.content);ModelWireOutputSchemas[kind].parse(decoded?.result);schemaStatus='VALID';}catch{schemaStatus='INVALID';}
...
const manifest={schemaVersion:1,approvedCommit:sha,evidenceMode:'live',purpose:'smoke',scenario:'real-schema-three-workers',
 ...
 bounds:{maxCalls:4,maxPosts:1,durationMs:1200000,maxOutputTokens:768,maxTokens:1500000},quietStopMs:null,
...
try{
 code=await new Promise(resolve=>{
  ...
  let kill;const maximum=setTimeout(()=>{child.kill('SIGTERM');kill=setTimeout(()=>child.kill('SIGKILL'),20000);},1280000);
  const progress=setInterval(()=>{try{const data=inspect(),done=records.filter(r=>r.summary);console.log(JSON.stringify({elapsedMs:Date.now()-started,modelRequests:records.length,responses:done.map(r=>r.summary),runs:data?.runs??[]}));
   // This bounded rerun asks only whether the redesigned model boundary can produce one Core-accepted result.
   if(data&&data.runs.some(r=>r.state==='DONE'))child.kill('SIGTERM');
   else if(data&&new Set(data.runs.filter(r=>r.state==='FAILED').map(r=>r.agentId)).size===3&&!data.runs.some(r=>r.state==='ACTIVE'))child.kill('SIGTERM');
  }catch{}},60000);
 ...
 const report={sourceSha:sha,applicationBase:'7e8cbd062081886b7802f1f5cde6e95b4a986f0e',protocol:'semantic-handles-v1',model,jsonMode:'schema',requestTimeoutMs:900000,leaseMs:30000,
  ...
  elapsedMs:result?.elapsedMs??Date.now()-started,bounds:manifest.bounds,requests:records.map(r=>r.summary??{kind:r.kind,owner:r.owner,requestBytes:r.requestBytes,incomplete:true}),
  maxRequestBytes:records.length?Math.max(...records.map(r=>r.requestBytes)):0,
