...
let stage='manifest',archivedSourceUsed=false,loadedImageUsed=false;
try{
  ...
  const loaded=JSON.parse(run('docker',['image','inspect',manifest.image.tag]))[0];assert.equal(loaded.Id,manifest.image.id);assert.equal(loaded.Config.User,'node');loadedImageUsed=true;
  stage='source-extract';
  run('tar',['-xzf',join(bundle,'source.tar.gz'),'-C',source,'--no-same-owner']);
  assert.equal(await digest(join(source,'package-lock.json')),manifest.lockfileSha256);archivedSourceUsed=true;
  ...
  stage='initial-start';await boot();
  ...
  stage='backup';const saved=storage('backup','/data/conversation.sqlite','/data/packaged-backup.sqlite');assert.equal(saved.integrity,'ok');
  stage='restore';const copy=storage('restore','/data/packaged-backup.sqlite','/data/packaged-restored.sqlite');assert.equal(copy.schemaVersion,saved.schemaVersion);
  stage='restored-start';compose('down');configure('/data/packaged-restored.sqlite');await boot();
  ...
  mkdirSync(resolve(report,'..'),{recursive:true});writeFileSync(report,JSON.stringify({mode:'packaged-application-e2e',outcome,failure,stage,sourceSha:manifest?.sourceSha,imageId:manifest?.image?.id,archivedSourceUsed,loadedImageUsed,originals:posts,backupRestored:restored,liveModelCalls:0,publishedToRegistry:false},null,2));
